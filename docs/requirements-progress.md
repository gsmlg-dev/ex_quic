...
| Abyss M5 dispatcher and external endpoint seam | Opt-in persistent dispatcher, monitored route registry, bounded writer, externally owned `QUIC.Endpoint`, legacy UDP path preserved | `gsmlg-dev/abyss@0de0b8a`; focused Abyss 34-test gate and ex_quic external endpoint tests pass; real QUIC-through-Abyss network acceptance remains pending |
