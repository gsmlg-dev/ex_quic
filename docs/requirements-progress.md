...
| M4 reliable streams | Connection-owned bounded stream state, bounded `Connection.open_stream/3` and `send_stream/5`, application STREAM/control frame codecs, flow-control admission and protected scheduler dispatch | focused stream/codec/scheduler checks and full 144-test local gate pass; independent multi-stream network transfer remains pending |
...
| Abyss M5 dispatcher and external endpoint seam | Opt-in persistent dispatcher, monitored route registry, bounded writer with local-send receipts, externally owned `QUIC.Endpoint`, legacy UDP path preserved | `gsmlg-dev/abyss@1e326b7` and `ex_quic@e9b08fd`; ex_quic 143-test gate and Abyss 501-test gate pass; refreshed cross-repo run passes two clients, single-close isolation, listener suspend/resume and restarted-listener admission; stream transfer and ordinary-UDP regression remain pending |
