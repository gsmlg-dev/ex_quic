...
| Abyss M5 dispatcher and external endpoint seam | Opt-in persistent dispatcher, monitored route registry, bounded writer, externally owned `QUIC.Endpoint`, legacy UDP path preserved | `gsmlg-dev/abyss@ace6cce` and `ex_quic@9a0308e`; Abyss full gate and ex_quic 141-test gate pass; one real QUIC-through-Abyss handshake passes; multi-client/lifecycle/stream regression cells remain pending |
