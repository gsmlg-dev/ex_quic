...
| Abyss M5 dispatcher and external endpoint seam | Opt-in persistent dispatcher, monitored route registry, bounded writer with local-send receipts, externally owned `QUIC.Endpoint`, legacy UDP path preserved | `gsmlg-dev/abyss@1e326b7` and `ex_quic@2ae78e4`; ex_quic 143-test gate passes; Abyss full gate and refreshed cross-repo run are environment-blocked; multi-client/lifecycle/stream regression evidence remains pending |
